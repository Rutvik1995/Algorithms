...
public class LongestCommonSubstring {
	...
	public static String computeLCSS(String a, String b) {
		if((a==null) || (b==null))
			return "";
		return computeLCSS(a.toCharArray(), a.length(), b.toCharArray(), b.length());
	}
	...
	private static String computeLCSS(char[] a, int lenA, char[] b, int lenB) {
		...
		System.out.println("Length of LCS = " + lcssLength + "Count = " + count);
		return null;
	}
	
	private static void computeLCSSLength(char[] a, int lenA, char[] b, int lenB, myInt tlen) {
		count++;
		if(lenA == 0 || lenB == 0) {
			tlen.a = 0;
			tlen.b = 0;
	 ...
	 */
	private static int computeLCSSLengthAnother(char[] a, int lenA, char[] b, int lenB) {
		count++;
		if(lenA == 0 || lenB == 0) {
			return 0;
		}
